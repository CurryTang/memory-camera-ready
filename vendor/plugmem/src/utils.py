...
def _normalize_models_url(base_url: str) -> str:
    ...
    return f"{base}/models" if not base.endswith("/models") else base


def _detect_openai_compat_model(base_url: str, api_key: str | None = None) -> str | None:
    try:
        headers = {}
        if api_key:
            headers["Authorization"] = f"Bearer {api_key}"
        response = requests.get(_normalize_models_url(base_url), headers=headers, timeout=15)
        response.raise_for_status()
        data = response.json().get("data") or []
        if data:
            return data[0].get("id")
    except Exception as e:
        print(f"Failed to detect model from {base_url}: {e}")
    return None
...
def call_qwen(prompt = None, messages = None, temperature=0, top_p=1.0, max_tokens=1024, token_usage_file=None, system_prompt="You are a helpful assistant."):
    VLLM_QWEN_API_KEY = os.environ["VLLM_QWEN_API_KEY"]
    QWEN_BASE_URL = os.environ["QWEN_BASE_URL"]
    # Cap max_tokens to fit within 60s timeout on slow servers (~30 t/s).
    max_tokens = min(int(max_tokens or 1024), 800)
    client = OpenAI(
    ...
    model_id = (
        ...
        or _detect_openai_compat_model(QWEN_BASE_URL, VLLM_QWEN_API_KEY)
        or "CalamitousFelicitousness/Qwen2.5-32B-Instruct-fp8-dynamic"
    )
    # Qwen3 ships with thinking-mode default ON, which (a) burns 1000+ tokens
    # on CoT before any output, and (b) wraps the actual answer in <think>
    # tags that defeat plugmem's `### Goal` / `### Experiential Insight`
    # regex extractor in structuring_inference.py. Disable thinking for all
    # plugmem calls — same trick as the memt vendor patch.
