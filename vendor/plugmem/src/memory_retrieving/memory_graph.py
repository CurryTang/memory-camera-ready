...
class MemoryGraph:
    ...
    def _parallel_get_embeddings(self, texts: List[str]) -> Dict[str, Any]:
        emb_cache: Dict[str, Any] = {}
        ordered_texts: List[str] = []
        for text in texts:
            normalized = "" if text is None else str(text)
            if normalized in emb_cache:
                continue
            emb_cache[normalized] = None
        ...
        batch_size = max(1, int(os.environ.get("LONGMEMEVAL_EMBED_BATCH", "64")))
        total_batches = (len(ordered_texts) + batch_size - 1) // batch_size
        if url:
            headers = {"Content-Type": "application/json"}
            for batch_idx, start in enumerate(range(0, len(ordered_texts), batch_size), start=1):
                batch = ordered_texts[start:start + batch_size]
                print(f"Embedding batch {batch_idx}/{total_batches} ({len(batch)} texts)", flush=True)
                ...
                if len(payload) != len(batch):
                    raise ValueError(f"Embedding response size mismatch: expected {len(batch)}, got {len(payload)}")
                for item, text_value in zip(payload, batch):
                    emb_cache[text_value] = item["embedding"]
            return emb_cache

        for batch_idx, text_value in enumerate(ordered_texts, start=1):
            if batch_idx == 1 or batch_idx % 100 == 0 or batch_idx == len(ordered_texts):
                print(f"Embedding item {batch_idx}/{len(ordered_texts)}", flush=True)
            emb_cache[text_value] = get_embedding(text_value) if text_value else get_embedding("")
        return emb_cache
    ...
    def build_mem_from_disk_lme_ver(self, file_path: str):
        ...
        for epis_item in epis_items:
            ...
            if not hasattr(episodic_node, "semantic_nodes"):
                episodic_node.semantic_nodes = []
        ...
        for procedural_item in procedural_items:
            ...
            if "return" not in procedural_item:
                procedural_item = dict(procedural_item)
                procedural_item["return"] = 0
    ...
    # ================== LongMemEval specific ==================
    def retrieve_episodic_memory(self, observation: str=None, **kwargs):
        return self.retrieve_episodic_nodes(observation=observation)
