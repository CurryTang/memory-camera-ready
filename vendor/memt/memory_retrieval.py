...
def _truncate_chars(text: str, max_chars: int = DEFAULT_RESULT_MAX_CHARS) -> str:
    text = str(text or "")
    if max_chars <= 0:
        return ""
    ...
    keep_chars = max(0, max_chars - len(TRUNCATION_MARKER))
    return text[:keep_chars].rstrip() + TRUNCATION_MARKER
...
class SearchSummaryTool(BaseTool):
    ...
    def _format_results(self, document: str, metadata: Dict, seen_turns: set = None, max_chars: int = DEFAULT_RESULT_MAX_CHARS) -> str:
        return _truncate_chars(self._format_with_source(document, metadata, seen_turns), max_chars)
    ...
    def __call__(self, query: str = "", sample_id: str = "", seen_turns: set = None, **kwargs):
        ...
        if not query:
            try:
                ...
                summary = self._format_results(doc, metadata, seen_turns)
...
class SearchFactsTool(BaseTool):
    ...
    def _format_results(self, res, seen_turns: set = None, max_chars: int = DEFAULT_RESULT_MAX_CHARS):
        ...
        for doc, meta in zip(res['documents'][0], res['metadatas'][0]):
            ...
            formatted_memories.append(_truncate_chars(f"{meta_str} {content}", max_chars))
            
        result = "Found Memories:\n" + "\n".join(formatted_memories)
        
        if new_source_turns:
             result += "\n\n----- NEW SOURCE CONTEXT -----\n" + _truncate_chars("\n".join(new_source_turns), max_chars)
...
class SearchPersonasTool(BaseTool):
    ...
    def _format_results(self, doc, meta, seen_turns: set = None, max_chars: int = DEFAULT_RESULT_MAX_CHARS):
        from utils import parse_memory_content
    ...
    def __call__(self, query: str = "", name: str = "", sample_id: str = "", seen_turns: set = None, **kwargs):
        ...
        if name:
            ...
            if res and res.get('documents') and res['documents'][0] and res["metadatas"] and res["metadatas"][0]:
                full_doc = self._format_results(res['documents'][0], res["metadatas"][0], seen_turns)
            ...
            if res and res.get('documents') and res['documents'][0]:
                 full_doc = self._format_results(res['documents'][0][0], res['metadatas'][0][0], seen_turns)
                 profile = full_doc
            else:
...
class SearchTurnsTool(BaseTool):
    ...
    def _format_results(self, turns: List[str], max_chars: int = DEFAULT_RESULT_MAX_CHARS) -> str:
        return "Relevant Turns:\n" + "\n".join(_truncate_chars(turn, max_chars) for turn in turns)
    ...
    def __call__(self, query: str = "", sample_id: str = "", top_k: int = 5, seen_turns: set = None, **kwargs):
        ...
        if new_turns:
            return self._format_results(new_turns), res['metadatas'][0]
        else:
...
class MemoryRetriever:
    def __init__(self, vector_db: VectorDBBase, llm_executor: LLMAPIClientBase, config: SystemConfig):
        ...
        self.max_context_tokens = getattr(config.memory, "max_context_tokens", 4096) if config else 4096
...
"""
    ...
    def _trim_messages_for_context(self, messages: List[Dict[str, str]]) -> List[Dict[str, str]]:
        ...
        interaction_messages = messages[2:][-4:]
        max_context_chars = max(0, int(self.max_context_tokens) * CHARS_PER_TOKEN)

        while len(interaction_messages) > 2 and sum(len(str(msg.get("content", ""))) for msg in interaction_messages) > max_context_chars:
            interaction_messages = interaction_messages[2:]
        ...
        for msg in reversed(interaction_messages):
            ...
            if len(content) > remaining_chars:
                content = _truncate_chars(content, remaining_chars)
            remaining_chars -= len(content)
            trimmed_msg = dict(msg)
            trimmed_msg["content"] = content
            trimmed_interactions.append(trimmed_msg)

        trimmed_interactions.reverse()
        return base_messages + trimmed_interactions
