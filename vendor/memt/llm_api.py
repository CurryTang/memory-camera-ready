...
class OpenAIAPIClient(LLMAPIClientBase):
    def __init__(self, model: str, api_key: str = None, base_url: str = None):
        ...
        self.model = model
        self.disable_thinking = os.environ.get("MEMT_DISABLE_THINKING", "1").lower() not in {"0", "false", "no"}
        self.max_tokens = int(os.environ.get("MEMT_MAX_TOKENS", "4096"))
        self.temperature = float(os.environ.get("MEMT_TEMPERATURE", "0.0"))
    ...
    def get_completion(self, 
        ...
        if self.disable_thinking and base_url and "api.openai.com" not in base_url:
    ...
    def get_completion(self, 
        ...
        if stop:
            try:
                ...
                raise ImportError("transformers StopStringCriteria is required for local stop-token generation.") from exc
...
class VLLMClient(LLMAPIClientBase):
    def __init__(self, model_name_or_path: str, device: str = "cuda:0", tensor_parallel_size: int = 1):
        if LLM is None or SamplingParams is None:
            raise ImportError("vllm is required for VLLMClient but is not installed.")
