...
class BCBRunner:
    ...
    @staticmethod
    def _aggregate_token_usage(token_log_path: str) -> Dict[str, Any]:
        summary: Dict[str, Any] = {
            "token_log_path": token_log_path,
            "entries": 0,
            "providers": {},
        }
        if not token_log_path or not os.path.exists(token_log_path):
            return summary
            ...
            for line in f:
                ...
                for key in [
                    ...
                    if isinstance(val, (int, float)):
                        provider_summary[key] += int(val)
                if entry.get("error"):
                    provider_summary["errors"] += 1
                model_name = str(entry.get("model") or "unknown")
                provider_summary["models"][model_name] = (
                    provider_summary["models"].get(model_name, 0) + 1
                )
        return summary
        ...
        *,
        ...
        for idx, task_id in enumerate(task_ids, start=1):
            ...
            sample = {
                ...
                "evaluation_time_s": round(eval_time_s, 6),
                "runtime_s": round(time.perf_counter() - sample_start, 6),
            }
        ...
        self._save_json(
            os.path.join(phase_dir, "metrics.json"),
            {
                ...
                "runtime_s": round(time.perf_counter() - phase_start, 6),
    ...
    def run(self) -> Dict[str, Any]:
        ...
        run_cfg = {
            ...
            "memory_enabled": self.mem is not None and self.retrieve_k > 0,
        ...
        token_usage_summary = self._aggregate_token_usage(
            os.path.join(self.output_dir, "token_usage.jsonl")
        )
        final = {
            "output_dir": self.output_dir,
            "epochs": epoch_summaries,
            "runtime_s": round(time.perf_counter() - run_start, 6),
