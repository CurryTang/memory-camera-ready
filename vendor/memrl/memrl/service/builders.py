...
_PROCEDURAL_PROMPT_OVERHEAD_TOKENS = 256
...
def _procedural_input_token_budget() -> int:
    """Return the opt-in procedural LLM input budget, or 0 when disabled."""
    requested_cap = _read_positive_int_env("MEMRL_PROCEDURAL_INPUT_CAP_TOKENS")
    model_max = (
        _read_positive_int_env("MEMRL_CHAT_CONTEXT_TOKENS")
        or _read_positive_int_env("MEMRL_MODEL_MAX_CONTEXT_TOKENS")
    )
    if requested_cap <= 0 and model_max <= 0:
        return 0

    budget = 30000
    if model_max > 0:
        budget = min(budget, max(1, model_max - 1024))
    if requested_cap > 0:
        budget = min(budget, requested_cap)
    return max(1, budget - _PROCEDURAL_PROMPT_OVERHEAD_TOKENS)


def _cap_procedural_llm_input(trajectory: str, *, callsite: str) -> str:
    """Cap trajectory text immediately before procedural/script LLM generation."""
    rendered = str(trajectory or "")
    max_tokens = _procedural_input_token_budget()
    if max_tokens <= 0:
        return rendered
    ...
    try:
        ...
        if len(tokens) <= max_tokens:
            return rendered
        capped = enc.decode(tokens[-max_tokens:])
        logger.warning(
            "Truncated MemRL procedural input at %s from %d to %d tokens",
            callsite,
            len(tokens),
            max_tokens,
        )
        ...
        if len(rendered) <= max_chars:
            return rendered
        logger.warning(
            "Truncated MemRL procedural input at %s from %d to %d chars",
            callsite,
            len(rendered),
            max_chars,
        )
...
class ScriptBuilder(MemoryBuilder):
    ...
    def build(self, task_description: str, trajectory: str) -> str:
        ...
        try:
            ...
            capped_trajectory = _cap_procedural_llm_input(
                trajectory,
                callsite="ScriptBuilder.build",
            )
            script = self.llm_provider.generate_script(capped_trajectory)
...
class ProceduralizationBuilder(MemoryBuilder):
    ...
    def build(self, task_description: str, trajectory: str) -> str:
        ...
        try:
            ...
            capped_trajectory = _cap_procedural_llm_input(
                trajectory,
                callsite="ProceduralizationBuilder.build",
            )
            script = self.llm_provider.generate_script(capped_trajectory)
            logger.debug(f"Generated script: {script[:100]}...")
