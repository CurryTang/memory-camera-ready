...
from .builders import get_builder, _cap_procedural_llm_input
...
class MemoryService:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.vector_dimension: int = int(kwargs.get("vector_dimension", 0) or 0)
        ...
        try:
            ...
            vector_dimension = int(getattr(self, "vector_dimension", 0) or 0)
            if not vector_dimension:
                try:
                    probe_embeddings = self.embedding_provider.embed(["dimension probe"])
                    vector_dimension = len(probe_embeddings[0]) if probe_embeddings else 3072
                except Exception:
                ...
                if script_start is not None and trajectory_start is not None:
                    ...
                    script = self.llm_provider.generate_script(
                        _cap_procedural_llm_input(
                            trajectory,
                            callsite="MemoryService.build_memory.fallback",
                        )
                    )
        ...
        if self.strategy_config.build == BuildStrategy.TRAJECTORY:
            ...
            if script_start is not None and trajectory_start is not None:
                ...
                script = self.llm_provider.generate_script(
                    _cap_procedural_llm_input(
                        trajectory,
                        callsite="MemoryService._prepare_memory_item.fallback",
                    )
                )
                ...
                "config": {
                    ...
                    "vector_db": {
                        ...
                        "config": {
                            ...
                            "vector_dimension": int(getattr(self, "vector_dimension", 0) or 3072),
                            "distance_metric": "cosine",
                            "path": qdrant_dir,
                        },
                    },
