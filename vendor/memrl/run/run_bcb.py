...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument(
        "--no_memory",
        action="store_true",
        help="Disable memory retrieval and memory updates for a clean no-memory baseline.",
    )
...
def main() -> None:
    ...
    mode_name = "nomemory" if args.no_memory else "memory"
    out_dir = out_root / "bigcodebench_eval" / f"{args.split}_{args.subset}" / mode_name
    ...
    # Providers
    token_log_path = str((run_dir / "token_usage.jsonl").resolve())
    llm = OpenAILLM(
    ...
    runner = BCBRunner(
        ...
        retrieve_k=(
            0 if args.no_memory else (args.retrieve_k if args.retrieve_k is not None else cfg.memory.k_retrieve)
        ),
